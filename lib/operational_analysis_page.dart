      ...
      child: Scaffold(
        ...
        body: StreamBuilder<Map<String, List<QueryDocumentSnapshot>>>(
          ...
          builder: (context, snapshot) {
            ...
            double singleRevenue = compSingles.fold(0, (sum, d) => sum + (double.tryParse(d['price']?.toString() ?? '0') ?? 0));

            // 2. قطاع المغسلة
            var compLaundry = laundryDocs.where((d) => d['status'] == 'completed').toList();
            double laundryRevenue = compLaundry.fold(0, (sum, d) => sum + (double.tryParse(d['totalPrice']?.toString() ?? '0') ?? 0));
            int totalLaundryPieces = compLaundry.fold(0, (sum, d) => sum + (int.tryParse(d['pieces']?.toString() ?? '0') ?? 0));
            ...
            double specialRevenue = compSpecial.fold(0, (sum, d) => sum + (double.tryParse(d['price']?.toString() ?? '0') ?? 0));

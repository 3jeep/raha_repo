...
class _MonthlyContractsPageState extends State<MonthlyContractsPage> {
  ...
  final Map<String, List<Map<String, dynamic>>> _visitsData = {};
  ...
  void _loadInitialData() {
    _db.collection('maids').snapshots().listen((snap) {
      if (mounted) {
        setState(() => _maids = snap.docs.map((d) => {'id': d.id, ...d.data()}).toList());
      }
    });
    _db.collection('vehicles').snapshots().listen((snap) {
      if (mounted) {
        setState(() => _vehicles = snap.docs.map((d) => {'id': d.id, ...d.data()}).toList());
      }
    });
    _fetchContracts();
  }
  ...
  void _fetchContracts() {
    _db.collection('contracts').orderBy('createdAt', descending: true).snapshots().listen((snap) async {
      ...
      for (var doc in snap.docs) {
        var data = doc.data();
        if (widget.adminType != 'super' && data['isPublished'] != true) {
          continue;
        }
  ...
  void _loadVisits(String contractId) async {
    ...
    if (mounted) {
      setState(() => _visitsData[contractId] = snap.docs.map((d) => {'id': d.id, ...d.data()}).toList());
    }
  }
  ...
  String _getTimer(Map order) {
    if (order['actualStartedAt'] == null || order['isPaused'] == true) {
      return "00:00:00";
    }
  ...
  Future<bool> _showConfirmDialog(String title, String content, Color confirmColor) async {
    return await showDialog(
      context: context,
      builder: (dContext) => Directionality(
        ...
        child: AlertDialog(
          ...
          actions: [
            TextButton(onPressed: () => Navigator.pop(dContext, false), child: const Text("إلغاء", style: TextStyle(color: Colors.grey))),
            ElevatedButton(
  ...
  void _finishVisitWithRating(Map b) {
    ...
    showModalBottomSheet(
      ...
      builder: (mContext) => StatefulBuilder(builder: (context, setModalState) {
        return Container(
            ...
            Row(children: [
              ...
              Expanded(flex: 2, child: ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  if (b['fcmToken'] != null) {
                    FcmSender.sendNotification(b['fcmToken'], "زيارة سعيدة ✨", "شكراً لتقييمك.");
                  }
                  if (mContext.mounted) {
    ...
    return Padding(
      ...
      child: Column(children: [
        Container(
          ...
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.blue.withAlpha(26))),
          child: TextField(
            onChanged: (v) {
              _searchQuery = v;
              _applyFiltering(_allContractsRaw);
  ...
  Widget _buildContractCard(Map<String, dynamic> b) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(40), boxShadow: [BoxShadow(color: Colors.black.withAlpha(13), blurRadius: 20)]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Container(
            ...
            decoration: BoxDecoration(color: Colors.blue.withAlpha(26), borderRadius: BorderRadius.circular(10)),
          ...
          if (widget.adminType == 'super')
             IconButton(icon: const Icon(Icons.delete_forever, color: Colors.redAccent, size: 20), onPressed: () async {
                 ...
                 if (confirm) {
                   _db.collection('contracts').doc(b['id']).delete();
                 }
             }),
        ]),
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              onPressed: () async {
                ...
                if (confirm) {
                  _db.collection('contracts').doc(b['id']).update({'isPublished': newStatus});
                }
              },
  ...
  void _handleStart(Map b) async {
    ...
    if (b['contractStartDate'] == null) {
      updates['contractStartDate'] = FieldValue.serverTimestamp();
    }

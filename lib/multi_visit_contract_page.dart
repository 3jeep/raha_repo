...
class _MultiVisitContractPageState extends State<MultiVisitContractPage> {
  ...
  DateTime? _startDate; 
  final List<String> _additionalDays = []; 
  ...
  Widget _buildFooter() {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(color: Colors.white, borderRadius: const BorderRadius.vertical(top: Radius.circular(45)), boxShadow: [BoxShadow(color: Colors.black.withAlpha(12), blurRadius: 20)]),
      child: Row(children: [
  ...
  Widget _genderOption(String label, String val, Color color) {
    ...
    return Expanded(child: InkWell(onTap: () => setState(() => _gender = val), child: Container(padding: const EdgeInsets.symmetric(vertical: 15), decoration: BoxDecoration(color: isSel ? color.withAlpha(25) : Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: isSel ? color : Colors.grey.shade200)), alignment: Alignment.center, child: Text(label, style: TextStyle(color: isSel ? color : Colors.grey, fontWeight: FontWeight.bold)))));
  }
  Widget _locationBtn() => SizedBox(width: double.infinity, child: ElevatedButton.icon(onPressed: _locating ? null : _handleGetLocation, icon: _locating ? const SizedBox(width: 15, height: 15, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.gps_fixed), label: const Text("تحديد موقعي عبر GPS"), style: ElevatedButton.styleFrom(backgroundColor: Colors.blue.withAlpha(25), foregroundColor: Colors.blue, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)))));
  Widget _contractSection(String t, String c, Color clr) => Container(margin: const EdgeInsets.only(bottom: 15), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: clr.withAlpha(12), borderRadius: BorderRadius.circular(15)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(t, style: TextStyle(color: clr, fontWeight: FontWeight.bold)), Text(c, style: TextStyle(color: clr.withAlpha(178), fontSize: 11))]));
